...
function Party() {
  ...
  const { party, participants, songs, currentSong, reveal, idleTimeout } =
    useParty(partyId!, participantId);
  ...
  return (
    <div>
      <div className="fixed top-4 left-4 right-4 z-50 bg-[#1a1a1a]/90 backdrop-blur-sm border border-[#2a2a2a] rounded-2xl px-5 py-3 flex items-center justify-between">
        ...
        {countdown ? (
          <p
            className={`text-sm transition-colors ${idleTimeout && idleTimeout - Date.now() <= 10000 ? "text-white font-bold" : "text-[#888]"}`}
          >
            party ends in {countdown}
          </p>
        ) : (
